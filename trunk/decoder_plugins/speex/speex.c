...
#if 0
/* Extract samples from the speex stream after ogg_stream_packetput() was
 * invoked. Return the number of samples extracted. */
static int extract_from_packet (struct spx_data *data, int16_t *buf,
		int buf_samples)
{
	int to_copy;
	int i;
	...
	assert (data != NULL);
	assert (buf != NULL);
	assert (buf_samples > 0);

	/* Copy Ogg packet to Speex bitstream */
	speex_bits_read_from(&data->bits, (char*)data->op.packet,
			data->op.bytes);

	for (i = 0; i != data->frames_per_packet; i++) {
		int ret;

		/* Decode frame */
		ret = speex_decode_int (data->st, &data->bits,
				data->remain_buf);
		...
		if (speex_bits_remaining(&data->bits) < 0) {
			decoder_error (&data->error, ERROR_STREAM, 0,
					"Decoding error: corrupted stream?\n");
			break;
		}
		...
		if (data->nchannels == 2)
			speex_decode_stereo_int (data->remain_buf,
					data->frame_size, &data->stereo);

		to_copy = buf_samples < data->frame_size * data->nchannels
			? buf_samples : data->frame_size * data->nchannels;
		memcpy (buf + buf_pos, data->remain_buf,
				to_copy * sizeof(int16_t));
		buf_pos += to_copy;
		buf_samples -= to_copy;

		data->remain_buf_fill = data->frame_size * data->nchannels
			- to_copy;
		if (data->remain_buf)
			memmove (data->remain_buf, data->remain_buf + to_copy *
					sizeof(int16_t),
					data->remain_buf_fill);

		speex_decoder_ctl (data->st, SPEEX_GET_BITRATE, &data->bitrate);
	}

	return buf_pos;
}

static int spx_decode (void *prv_data, char *buf, int buf_len,
		struct sound_params *sound_params)
{
	struct spx_data *data = (struct spx_data *)prv_data;
	int16_t *out = (int16_t *)buf;
	int out_pos = 0;
	int out_len = buf_len / sizeof(int16_t);

	sound_params->channels = data->nchannels;
	sound_params->rate = data->rate;
	sound_params->fmt = SFMT_S16 | SFMT_NE;

	if (data->remain_buf_fill) {
		int to_copy = data->remain_buf_fill < out_len
			? data->remain_buf_fill : out_len;

		memcpy (out, data->remain_buf, out_len * sizeof(int16_t));
		out_pos += to_copy;
		data->remain_buf_fill -= to_copy;
	...
	while (out_pos < out_len && !io_eof(data->stream)) {
		char *sync_buf;
		int nb_read;

		/* Get the ogg buffer for writing */
		sync_buf = ogg_sync_buffer (&data->oy, 200);

		/* Read bitstream from input file */
		nb_read = io_read(data->stream, sync_buf, 200);      
		if (nb_read < 0) {
			decoder_error (&data->error, ERROR_FATAL, 0,
					"Can't open speex file: IO error: %s",
					io_strerror(data->stream));
		...
		while (ogg_sync_pageout(&data->oy, &data->og) == 1) {
			...
			while (ogg_stream_packetout(&data->os, &data->op)
				...
				out_pos += extract_from_packet (data,
						out + out_pos,
						out_len - out_pos);
			}
		}
